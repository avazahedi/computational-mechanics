...
# Grab the 1st column
X[:, 0]
# Grab 2nd element in 2nd column
X[1, 1]
```
...
arr_res34 = arr_3+arr_4
```

```{code-cell} ipython3
%%time
lst_3 = random.sample(range(10000), 10000)
lst_4 = random.sample(range(10000), 10000)
res_lst34 = []
for i in range(10000):
    res_lst34.append(lst_3[i] + lst_4[i])
```
...
```{code-cell} ipython3
plt.plot(xarray, xarray*2, color='yellow', linestyle=':', label='$2x$')
plt.plot(xarray, xarray**4, color='red', linestyle='-', label='$x^4$')
plt.legend(loc='best');
```
...
plt.plot(x, cosx, color='blue', linestyle='-', label='$cos(x)$')
plt.legend(loc='best');
```
